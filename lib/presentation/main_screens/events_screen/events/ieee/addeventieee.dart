...
import 'Event_Service_ieee.dart';
...
class addeventieee extends StatefulWidget {
  const addeventieee({super.key});

  @override
  State<addeventieee> createState() => _addeventieeeState();
}
...
EventServiceieee eventService = EventServiceieee();
...
class _addeventieeeState extends State<addeventieee> {
  File? _image;

  void countDocuments() async {
    QuerySnapshot doc =
        await FirebaseFirestore.instance.collection('eIeee').get();
  ...
  validateAndUpload() {
    ...
    if (_image != null) {
      ...
      UploadTask task =
          storage.child('V-CEC_2.0.0/IEEE/$picture').putFile(_image!);
