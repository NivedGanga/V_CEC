...
import 'package:v_cec_firebse1/presentation/main_screens/events_screen/events/iedc/Event_Service_iedc.dart';

class addeventiedc extends StatefulWidget {
  const addeventiedc({super.key});

  @override
  State<addeventiedc> createState() => _addeventiedcState();
}
...
EventServiceiedc eventService = EventServiceiedc();
...
class _addeventiedcState extends State<addeventiedc> {
  File? _image;
  var size;
  void countDocuments() async {
    QuerySnapshot doc =
        await FirebaseFirestore.instance.collection('eIedc').get();
  ...
  validateAndUpload() {
    ...
    if (_image != null) {
      ...
      UploadTask task =
          storage.child('V-CEC_2.0.0/IEDC/$picture').putFile(_image!);
